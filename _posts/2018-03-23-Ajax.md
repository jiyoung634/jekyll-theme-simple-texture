				...
				xmlhttp.onreadystatechange = function() {
					if (this.readyState == 4 && this.status == 200) {
						...
						for (m in obj.members) {
							txt += "<table class=\"table table-striped\"><tbody>";
							txt += "<tr><td colspan=\"2\" style=\" text-align:center\"><img src=\""+obj.members[m].picture+"\" style=\"width:20%\">"+"</td></tr>";
...
<%
	...
	if (list.size() > 0) {
		for (int i = 0; i < list.size(); ++i) {
			Member m = list.get(i);
			sb.append(String.format(
					"{\"mid_\":\"%s\",\"name_\":\"%s\", \"phone\":\"%s\", \"email\":\"%s\", \"regDate\":\"%s\", \"deptName\":\"%s\", \"picture\":\"%s\"}",
					m.getMid_(), m.getName_(), m.getPhone(), m.getEmail(), m.getRegDate(), m.getDeptName(),
					contextRoot + "/resources/img/avatar.png"));
			if(i<list.size()-1) {
				sb.append(",");
